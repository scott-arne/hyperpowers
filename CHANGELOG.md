...
- **Release gating.** Offline: 19/19 suites plus `test-sdd-contract.sh` at 63 assertions. Live: `sdd-plan-scoped-scratch` pass, `sdd-rejects-extra-features` pass (0 failed checks), and the live bash suite `test-subagent-driven-development.sh` 15/15. `sdd-unified-fix-loop` returned indeterminate on fixture and stub defects rather than skill behavior. The live-agent integration test did not complete within its own 1800-second ceiling and is recorded as inconclusive.
