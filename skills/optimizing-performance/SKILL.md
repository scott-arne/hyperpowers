...
2. **Plan the batch.** Filter the candidates to the independent, high-confidence ones. Turn them into a plan with hyperpowers:writing-plans, **one candidate per task**, each task carrying its hypothesis, expected payoff-vs-bound, and the *exact* benchmark command. Coupled or speculative candidates wait for the re-profile round.
