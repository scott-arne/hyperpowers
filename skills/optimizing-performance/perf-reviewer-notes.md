...
Report the evidence the coordinator needs for the keep / revert / tie-break call: the measured delta, run-to-run variance, complexity cost, and correctness result. Compare these against the candidate's producer-side estimates carried in the task brief — flag a large gap between the measured complexity and the original `complexity-cost`, or between the measured delta and the `expected-payoff-vs-bound`. State what you can and cannot verify from the diff and the report. **Do not** make the keep/revert decision yourself — the coordinator holds the shared baseline and the cross-attempt view.
