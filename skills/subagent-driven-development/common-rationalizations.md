...
| "I'll fix it myself, dispatching is overhead" | Controller fixes pollute your context and skip review. Outside the de-minimis exception (one finding: at most 3 fully-specified lines in one file, no new logic — and it still costs a fix round, still runs the covering command with the fix report appended, disclosed in the ledger, re-review still runs), that is rationalization. Resume the implementer. |
